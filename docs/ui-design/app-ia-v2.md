...
- The on-chain trade price chart (section 4.8, when the product has a price series) is the first card of the left column, below the Pyth block on mobile; the difference between its last trade and the Pyth reference (with both times and the per-share unit) is shown once, in the price comparison panel right after the chart, not in the chart heading.
...
- Product page, price comparison panel (last trade vs Pyth): the difference only when the last trade has a value for one share (`usdcPerUnderlyingShare`, the multiplier at that trade is known); that value is compared with the Pyth reference price. Otherwise no difference is shown, with a note that the multiplier history is unavailable.
