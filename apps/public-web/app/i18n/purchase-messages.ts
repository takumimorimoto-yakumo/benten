...
const purchaseEn: PurchaseCopy = {
  ...
  pay: {
    ...
    route: (symbol) => `${symbol} to USDC to NVDAx: two fixed Meteora DLMM pools, in one transaction you approve in your wallet.`, firstLeg: "First pool: expected USDC", firstLegValue: (pay, usdc) => `${pay} = ${usdc}`, usdcIn: "USDC into the NVDAx pool", usdcInNote: "The first pool's minimum after slippage. Any USDC above it stays in your wallet.", limitBasis: (usdc, limit) => `Limit check: the first pool expects to return ${usdc} USDC for this amount. The limit is ${limit} USDC per transaction.`, firstLegFee: (amount) => `Pool fee ${amount}`, nvdaxPoolFee: "NVDAx pool fee", nvdaxPoolImpact: "NVDAx pool price impact", accountCreation: "This transaction may also create the token accounts it needs: NVDAx, USDC, and wrapped SOL when paying with SOL. Your wallet shows any one-time SOL deposit; the wrapped SOL account is closed and its deposit returned in the same transaction.",
...
const purchaseJa: PurchaseCopy = {
  ...
  pay: {
    ...
    route: (symbol) => `${symbol}からUSDC、USDCからNVDAxへ: 2つの固定されたMeteora DLMMプールを、ウォレットで承認する1つのトランザクションで通ります。`, firstLeg: "1つ目のプール: 受け取る見込みのUSDC", firstLegValue: (pay, usdc) => `${pay} = ${usdc}`, usdcIn: "NVDAxプールに入れるUSDC", usdcInNote: "1つ目のプールのスリッページ後の最低額です。これを超えたUSDCはウォレットに残ります。", limitBasis: (usdc, limit) => `上限の確認: 1つ目のプールはこの数量で ${usdc} USDCを返す見込みです。上限は1回の取引で ${limit} USDCです。`, firstLegFee: (amount) => `プール手数料 ${amount}`, nvdaxPoolFee: "NVDAxプールの手数料", nvdaxPoolImpact: "NVDAxプールの価格インパクト", accountCreation: "このトランザクションは、必要なトークンアカウント(NVDAx、USDC、SOLで支払う場合はラップドSOL)も作成することがあります。1回限りのSOLのデポジットはウォレットに表示されます。ラップドSOLのアカウントは同じトランザクション内で閉じられ、そのデポジットは戻ります。",
...
const purchaseKo: PurchaseCopy = {
  ...
  pay: {
    ...
    route: (symbol) => `${symbol}에서 USDC, USDC에서 NVDAx로: 두 개의 고정된 Meteora DLMM 풀을 지갑에서 승인하는 하나의 트랜잭션으로 거칩니다.`, firstLeg: "첫 번째 풀: 예상 USDC", firstLegValue: (pay, usdc) => `${pay} = ${usdc}`, usdcIn: "NVDAx 풀에 들어가는 USDC", usdcInNote: "첫 번째 풀의 슬리피지 적용 후 최소 금액입니다. 이를 넘는 USDC는 지갑에 남습니다.", limitBasis: (usdc, limit) => `한도 확인: 첫 번째 풀은 이 수량에 대해 ${usdc} USDC를 돌려줄 것으로 예상합니다. 한도는 거래당 ${limit} USDC입니다.`, firstLegFee: (amount) => `풀 수수료 ${amount}`, nvdaxPoolFee: "NVDAx 풀 수수료", nvdaxPoolImpact: "NVDAx 풀 가격 영향", accountCreation: "이 트랜잭션은 필요한 토큰 계정(NVDAx, USDC, SOL로 지불할 때는 래핑된 SOL)도 생성할 수 있습니다. 1회성 SOL 예치금은 지갑에 표시됩니다. 래핑된 SOL 계정은 같은 트랜잭션 안에서 닫히고 예치금은 돌아옵니다.",
...
const purchaseZhHans: PurchaseCopy = {
  ...
  pay: {
    ...
    route: (symbol) => `${symbol} 兑换为 USDC，再兑换为 NVDAx：经过两个固定的 Meteora DLMM 池，在你钱包中批准的同一笔交易内完成。`, firstLeg: "第一个池：预计收到的 USDC", firstLegValue: (pay, usdc) => `${pay} = ${usdc}`, usdcIn: "进入 NVDAx 池的 USDC", usdcInNote: "第一个池扣除滑点后的最低数额。超出部分的 USDC 留在你的钱包中。", limitBasis: (usdc, limit) => `上限核对：第一个池预计为此数量返回 ${usdc} USDC。每笔交易上限为 ${limit} USDC。`, firstLegFee: (amount) => `池手续费 ${amount}`, nvdaxPoolFee: "NVDAx 池手续费", nvdaxPoolImpact: "NVDAx 池价格影响", accountCreation: "此交易可能还会创建所需的代币账户：NVDAx、USDC，以及用 SOL 支付时的包装 SOL。钱包会显示一次性的 SOL 押金；包装 SOL 账户会在同一笔交易内关闭，其押金会退回。",
...
const purchaseZhHant: PurchaseCopy = {
  ...
  pay: {
    ...
    route: (symbol) => `${symbol} 兌換為 USDC，再兌換為 NVDAx：經過兩個固定的 Meteora DLMM 池，在你錢包中核准的同一筆交易內完成。`, firstLeg: "第一個池：預計收到的 USDC", firstLegValue: (pay, usdc) => `${pay} = ${usdc}`, usdcIn: "進入 NVDAx 池的 USDC", usdcInNote: "第一個池扣除滑價後的最低數額。超出部分的 USDC 留在你的錢包中。", limitBasis: (usdc, limit) => `上限核對：第一個池預計為此數量返回 ${usdc} USDC。每筆交易上限為 ${limit} USDC。`, firstLegFee: (amount) => `池手續費 ${amount}`, nvdaxPoolFee: "NVDAx 池手續費", nvdaxPoolImpact: "NVDAx 池價格影響", accountCreation: "此交易可能還會建立所需的代幣帳戶：NVDAx、USDC，以及用 SOL 支付時的包裝 SOL。錢包會顯示一次性的 SOL 押金；包裝 SOL 帳戶會在同一筆交易內關閉，其押金會退回。",
...
export const PURCHASE_MESSAGES: Record<PublicWebLocale, PurchaseCopy> = { en: purchaseEn, ja: purchaseJa, ko: purchaseKo, "zh-Hans": purchaseZhHans, "zh-Hant": purchaseZhHant };

export function purchaseMessagesFor(locale: PublicWebLocale): PurchaseCopy {
  return PURCHASE_MESSAGES[locale];
}
