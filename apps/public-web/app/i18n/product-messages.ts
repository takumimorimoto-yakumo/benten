...
const en: ProductCopy = {
  ...
  product: {
    ...
    capability: {
      buyable: "Buy in Benten with USDC", buy: (symbol) => `Buy ${symbol}`,
      notBuyableHeading: "Not buyable in Benten", notBuyableBody: "Benten builds purchases only for NVDAx with USDC, through one fixed pool.",
...
const ja: ProductCopy = {
  ...
  product: {
    ...
    capability: {
      buyable: "BentenでUSDCを使って購入できます", buy: (symbol) => `${symbol}を購入`,
      notBuyableHeading: "Bentenでは購入できません", notBuyableBody: "Bentenが購入を組み立てるのは、1つの固定プールを通じてUSDCでNVDAxを購入する場合だけです。",
...
const ko: ProductCopy = {
  ...
  product: {
    ...
    capability: {
      buyable: "Benten에서 USDC로 구매할 수 있습니다", buy: (symbol) => `${symbol} 구매`,
      notBuyableHeading: "Benten에서 구매할 수 없습니다", notBuyableBody: "Benten은 하나의 고정된 풀을 통해 USDC로 NVDAx를 구매하는 경우에만 구매를 구성합니다.",
...
const zhHans: ProductCopy = {
  ...
  product: {
    ...
    capability: {
      buyable: "可在 Benten 中用 USDC 购买", buy: (symbol) => `购买 ${symbol}`,
      notBuyableHeading: "无法在 Benten 中购买", notBuyableBody: "Benten 只为通过一个固定池用 USDC 购买 NVDAx 构建交易。",
...
const zhHant: ProductCopy = {
  ...
  product: {
    ...
    capability: {
      buyable: "可在 Benten 中用 USDC 購買", buy: (symbol) => `購買 ${symbol}`,
      notBuyableHeading: "無法在 Benten 中購買", notBuyableBody: "Benten 只為透過一個固定池用 USDC 購買 NVDAx 建立交易。",
